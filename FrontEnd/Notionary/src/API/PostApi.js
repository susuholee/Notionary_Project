...
const AllCategoryPost = async () => {
  const { data } = await axios.get(`${API_URL}/api/main`);
...
const SubCategoryPost = async ({ category_name, SubCategory }) => {
  ...
  const { data } = await axios.post(`${API_URL}/api/main/subpost`, {
    category_name,
    SubCategory,
  });
...
const EtcCategoryPost = async () => {
  const { data } = await axios.get(`${API_URL}/api/main/etc`);
  console.log("기타 카테고리 응답 데이터:", data);
  return data;
};
const CreatePost = async (formData) => {
  console.log(formData, 'formData')
  const { data } = await axios.post(`${API_URL}/api/post`, formData);
...
const UpdatePost = async ({post_id, formData}) => {
  const { data } = await axios.put(`${API_URL}/api/post/edit/${post_id}`, formData);
  console.log("업데이트 결과 데이터:", data);
  return data;
};

const GetPostById = async (post_id) => {
  const { data } = await axios.get(`${API_URL}/api/post/${post_id}`);
  console.log("수정할 게시글 ID  데이터" , data)
  return data;
};

const GetWorkSpace = async (uid) => {
  const { data } = await axios.post(`${API_URL}/api/post/getWorkspace`, {
    uid: uid,
  });
