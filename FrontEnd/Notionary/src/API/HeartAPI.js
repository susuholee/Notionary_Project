...
const CreateHeart = async ({ uid, post_id }) => {
  const { data } = await axios.post(`${API_URL}/api/main/heart`, {
    uid, post_id
  });
...
const GetHeartUser = async (post_id) => {
  const { data } = await axios.get(`${API_URL}/api/main/heart/${post_id}`)
  console.log("좋아요 유저 조회 응답",data)
  return data
}

const DeleteHeart = async ({ uid, post_id }) => {
  const {data} = await axios.delete(`${API_URL}/api/main/heartDelete`, {
     data: { uid, post_id }
  });
